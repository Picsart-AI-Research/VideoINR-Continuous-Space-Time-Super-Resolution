...
class LunaTokis(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10):
        ...
        self.feat_imnet = Siren(in_features=200, out_features=128, hidden_features=[64, 64, 64, 256], 
                  hidden_layers=3, outermost_linear=True)
        self.flow_imnet = Siren(in_features=128+192+6+1+2, out_features=4, hidden_features=[64, 64, 64, 256], 
                  hidden_layers=3, outermost_linear=True)
        self.encode_imnet = Siren(in_features=128*2+192*2+12, out_features=27, hidden_features=[64, 64, 64, 256, 256], 
    ...
    def decoding(self, times=None, scale=None):
        ...
        bs, C, H, W = feat.shape # [Batch_size, channel_num, H, W]
        if scale is None:
            HH, WW = H * 4, W * 4
        ...
        for c in range(len(times)):
            ...
            inp = torch.cat([q_feat, q_inp, rel_coord], dim=-1)
            HRfeat = self.feat_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1)
            HRfeat = HRfeat.permute(0, 2, 1).view(bs, 128, HH, WW)
            q_feat = F.grid_sample(
            ...
            rel_coord[:, :, 0] *= feat.shape[-2]
            rel_coord[:, :, 1] *= feat.shape[-1]
            flow_inp = torch.cat([q_feat, q_feat0, q_inp, rel_coord, pe_coord], dim=-1)
            ...
            q_feat3 = F.grid_sample(
                ...
                mode='bilinear', align_corners=False)[:, :, 0, :] \
                .permute(0, 2, 1)
            q_inp1 = F.grid_sample(
                self.inp.view(feat.shape[0], -1, feat.shape[2], feat.shape[3]), grid.flip(-1).unsqueeze(1),
                mode='bilinear', align_corners=False)[:, :, 0, :] \
                .permute(0, 2, 1)
            ...
            q_feat4 = F.grid_sample(
                ...
                mode='bilinear', align_corners=False)[:, :, 0, :] \
                .permute(0, 2, 1)
            q_inp2 = F.grid_sample(
                self.inp.view(feat.shape[0], -1, feat.shape[2], feat.shape[3]), grid.flip(-1).unsqueeze(1),
                mode='bilinear', align_corners=False)[:, :, 0, :] \
                .permute(0, 2, 1)
        
            inp = torch.cat([q_feat1, q_feat3, q_inp1, q_feat2, q_feat4, q_inp2], dim=-1)
            pred = F.fold(self.encode_imnet(inp.view(bs * qs, -1)).view(bs, qs, -1).permute(0, 2, 1), output_size=(HH, WW), kernel_size=3, padding=1)
            preds.append(pred)
        return preds
    
    def decoding_test(self, times=None, scale=None):
        '''
        In order to avoid CUDA out of memory when testing,
        we do not predict RGB values of all pixels in the target.
        Instead we run the model multiple times, taking different
        part of pixels as input.

        Here the number of model forward is hardcoded as 3. You may
        modify the code to fulfill your requirement in testing. 
        '''
        feat = torch.cat([self.feat[:, 0], self.feat[:, 1], self.feat[:, 2]], dim=1)
        bs, C, H, W = feat.shape # [Batch_size, channel_num, H, W]
        ...
        for c in range(len(times)):
            ...
            inp_p1 = torch.cat([q_feat1[:, :qs1], q_feat3[:, :qs1], q_inp1[:, :qs1], q_feat2[:, :qs1], q_feat4[:, :qs1], q_inp2[:, :qs1]], dim=-1)
            pred_p1 = self.encode_imnet(inp_p1.view(bs * qs1, -1)).view(bs, qs1, -1)
            del inp_p1
            torch.cuda.empty_cache()
            inp_p2 = torch.cat([q_feat1[:, qs1:qs1+qs2], q_feat3[:, qs1:qs1+qs2], q_inp1[:, qs1:qs1+qs2], q_feat2[:, qs1:qs1+qs2], q_feat4[:, qs1:qs1+qs2], q_inp2[:, qs1:qs1+qs2]], dim=-1)
            pred_p2 = self.encode_imnet(inp_p2.view(bs * qs2, -1)).view(bs, qs2, -1)
            del inp_p2
            torch.cuda.empty_cache()
            inp_p3 = torch.cat([q_feat1[:, qs1+qs2:], q_feat3[:, qs1+qs2:], q_inp1[:, qs1+qs2:], q_feat2[:, qs1+qs2:], q_feat4[:, qs1+qs2:], q_inp2[:, qs1+qs2:]], dim=-1)
            pred_p3 = self.encode_imnet(inp_p3.view(bs * qs3, -1)).view(bs, qs3, -1)
            del inp_p3
            torch.cuda.empty_cache()
            pred = torch.cat([pred_p1, pred_p2, pred_p3], dim=1)
            pred = F.fold(pred.permute(0, 2, 1), output_size=(HH, WW), kernel_size=3, padding=1)
            preds.append(pred)
        return preds
